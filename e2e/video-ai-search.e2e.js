...
beforeAll(async () => {
  ...
  if (!merksemRelayVideo) {
    throw new Error('Missing seeded Merksem 4x400 mixed video');
  }
  merksemCompetition = catalog.competitions.find((entry) => entry.id === merksemRelayVideo.event_id)
    || catalog.competitions.find((entry) => /merksem/i.test(String(entry.event_name || entry.name || '')))
    || null;
